Veewee::Session.declare({
  ...
  :boot_wait => "3",
  :boot_cmd_sequence => [
    '<Wait><Esc><Wait>core<Enter>','<Wait>' * 5, # CorePlus menu
    ...
    'tce-load -wi tc-install<Enter>', '<Wait>'*40, # perl5, tc-install
    'sudo touch /etc/sysconfig/desktop<Enter>',
    'sudo tc-install.sh<Enter>', '<Wait>'*5,
    'c<Enter><Wait>', 'f<Enter><Wait>',
    '1<Enter><Wait>', '1<Enter><Wait>',
    'n<Enter><Wait>', 'g<Enter><Wait>',
    'n<Enter>', 'n<Enter>',
    'n<Enter>', 'n<Enter>',
    'n<Enter>', 'n<Enter>', # remaster,kmaps
    '3<Enter><Wait>', 'waitusb=0 showapps noswap fltk_topside noutc tce=sda1<Enter>', # vga=normal
    'y<Enter>', '<Wait>'*50, '<Enter>', # formatting...

    'sudo mkdir -p /mnt/sda1<Enter>',
    'sudo mount /dev/sda1 /mnt/sda1<Enter>',
    
    'sudo mount /dev/sr0 /mnt/sr0<Enter>', # may fail
    'cd /mnt/sr0/boot<Enter>',
    'sudo mkdir -p /mnt/sda1/boot/<Enter>',
    'sudo cp core.gz vmlinuz /mnt/sda1/boot/<Enter><Wait>cd<Enter>',
    ...
    '  linux (hd0,1)/boot/vmlinuz waitusb=0 noswap root=/dev/sda1 tce=sda1 text<Enter>',
    '  initrd (hd0,1)/boot/core.gz<Enter>',
    '}<Enter>',
    'EOF<Enter>',
    'sudo mkdir -p /mnt/sda1/tce<Enter>',
    ...
    'set timeout=1<Enter>',
    'insmod ext2<Enter>',
    'menuentry \'Core Linux with X Windows\' {<Enter>',
    '  set root=(hd0,1)<Enter>',
    '  linux (hd0,1)/boot/vmlinuz noswap root=/dev/sda1 waitusb=0 showapps fltk_topside noutc tce=sda1<Enter>',
    '  initrd (hd0,1)/boot/core.gz<Enter>',
    '}<Enter>',
    'menuentry \'Core Linux\' {<Enter>',
    '  set root=(hd0,1)<Enter>',
